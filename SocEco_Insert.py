...
df_eco_reg[15] = df_eco_reg[15].fillna(method="ffill") 
df_eco_reg[16] = df_eco_reg[16].fillna(method="ffill")
...
def insert(dataframe, command, liste, indic, nom=""):
    ...
    for i in range(1,dataframe.shape[0]):
        ...
        if dataframe.iloc[i][0] == "M" or dataframe.iloc[i][0] == "F" or dataframe.iloc[i][0] == "P" or dataframe.iloc[i][0] == "FHM " or dataframe.iloc[i][0] == "F " :
            continue
        for j in liste:
...
# Insertion dans la table Indicateur Région et Indicateur Departement des données sociales.
...
for i in range(2,16):
    ...
    if i in [2,3,4]:
        ...
        insert(df_soc_dep, command, columns_list,soc_indic, "indicateurD")
    elif i in [5,6,7]:
...
# Insertion dans la table Indicateur Région et Indicateur Departement des données économiques.
...
for i in range(2,16):
    ...
    if i in [15,16]:
        command = "INSERT INTO indicateurR(annee,IdI,IdR,valeur,sexe) VALUES (%s,%s,%s,%s,0)"
        insert(df_eco_reg, command, columns_list,eco_indic, "indicateurR")
    else:
        command = "INSERT INTO indicateurR(annee,IdI,IdR,valeur,sexe) VALUES (%s,%s,%s,%s,0)"
        insert(df_eco_reg, command, columns_list,eco_indic, "indicateurR")
        command = "INSERT INTO indicateurD(annee,IdI,IdD,valeur,sexe) VALUES (%s,%s,%s,%s,0)"
        insert(df_eco_dep, command, columns_list,eco_indic, "indicateurD")
